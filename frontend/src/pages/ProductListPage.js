...
import { MOVIE_CREATE_RESET } from '../constants/productConstants';
...
const ProductListPage = ({ history, match }) => {
	...
	const { loading, products, error, pages, page } = movieList;
	...
	useEffect(() => {
		...
		dispatch({ type: MOVIE_CREATE_RESET }); //reset the new product detail
		if (successCreate)
			history.push(`/admin/product/${createdProduct._id}/edit`);
		else dispatch(listMovie('', pageNumber, 10)); // 3rd parameter is the no of products to be listed per page
	}, [
		dispatch,
		history,
		userInfo,
